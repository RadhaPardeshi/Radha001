...
public class ViewOpenAccountsOfCutomerwithdetails {
		...
		public static void main(String[] args) throws IOException, AWTException, Throwable 
		{
			...
			String resulta = driver.findElement(By.xpath("//table/tbody/tr/td[last()]")).getText();
			...
			driver.findElement(By.name("view_cust_by_ac")).sendKeys(applID);
			if(resulta.contains("ACTIVE"))
			{
				System.out.println("Customer is present with account status as active");

			}
			else
			{
				System.out.println("Customer is not present shows customer not found popup");
